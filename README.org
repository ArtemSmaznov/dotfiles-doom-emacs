...
** Search engines
#+begin_src emacs-lisp
(cl-pushnew '("Brave" "https://search.brave.com/search?q=%s") +lookup-provider-url-alist)
#+end_src
...
(use-package! org
  ...
  :init
  (setq
   org-directory (if (featurep :system 'macos) "~/docs/notes/"
  ...
  (dolist (item '(("el" . "src emacs-lisp")
                  ...
                  ("sh" . "src shell")
                  ("ya" . "src yaml")))
    (cl-pushnew item org-structure-template-alist))

  ;; extra org structure templates
  (dolist (item '(("conf-unix" . conf-unix)
                  ("toml"      . conf-toml)))
    (cl-pushnew item org-src-lang-modes)))
#+end_src
...
(use-package! calfw
  ...
  (map! :leader
        :prefix "o"
        :desc "Calendar" :e "c" #'calfw-org-open-calendar)

  :config
  (map! :map calfw-calendar-mode-map
        :m "C-j" #'calfw-navi-next-month-command
        :m "C-k" #'calfw-navi-previous-month-command
        :m "C-." #'calfw-navi-goto-today-command
        :m "0"   #'calfw-navi-goto-week-begin-command
        :m "gd"  #'calfw-org-goto-date
        :m "zd"  #'calfw-change-view-day
        :m "zw"  #'calfw-change-view-week
        :m "zm"  #'calfw-change-view-month
        :m "zt"  #'calfw-change-view-two-weeks
        :m "T"   #'calfw-change-view-two-weeks)) ;; not active due to evil-snipe and evil-find-char
#+end_src
  ...
  (setq
   ...
   org-caldav-calendars `((:calendar-id "personal" :select-tags ("calgnr")
                          ...
                          (:calendar-id "travel" :select-tags ("caltrvl")
                           :inbox ,(expand-file-name "notes.org" org-directory)
                           :files (,(expand-file-name "notes.org" org-directory)
                                   ,(expand-file-name "archive.org" org-directory)))
...
(use-package! magit
  :defer t
  :init
  (defun my/magit-yank-current-branch ()
  ...
  (map! :map magit-mode-map
        :prefix "y"
        :desc "Yank currect branch" "B" #'my/magit-yank-current-branch)
...
(use-package! code-review
  ...
  :defer t
  :hook
  (code-review-mode . (lambda () (persp-add-buffer (current-buffer)))))
...
(use-package! pr-review
  ...
  (map! :map magit-mode-map
        ...
        "c" #'pr-review-select-commit
        "C" #'pr-review-context-comment
        "d" #'pr-review-ediff-file
        "e" #'pr-review-context-edit
        "f" #'pr-review-view-file
        "l" #'pr-review-set-labels
        "r" #'pr-review-submit-review))
#+end_src
...
(use-package! magit-todos
  ...
  (dolist (item '("Basemark*/"
                  ...
                  "vivaldi*/"
                  "{emacs,doom}/"))
    (cl-pushnew item magit-todos-exclude-globs)))
#+end_src
...
(use-package! rotate-text
  ...
  (dolist (item '(("t" "nil")
                  ...
                  ("black" "gray" "white")
                  ("red" "green" "blue")))
    (cl-pushnew item rotate-text-words)))
#+end_src
...
(use-package! lsp-ui
  ...
  (map! :map lsp-ui-doc-mode-map
        :desc "Focus UI Doc frame" :neg "<C-tab>" #'lsp-ui-doc-focus-frame)
  (map! :map lsp-ui-doc-frame-mode-map
        :desc "Unfocus UI Doc frame" :neg "q" #'lsp-ui-doc-unfocus-frame)

  :config
  (setq ;; 1. Symbol highlighting
   lsp-enable-symbol-highlighting t)

  (setq ;; 2. UI Doc hover dialogs
   lsp-ui-doc-enable nil
   lsp-ui-doc-delay 0.75
   lsp-ui-doc-show-with-cursor t
   lsp-ui-doc-show-with-mouse nil ;; requires restart

   lsp-ui-doc-max-width 150
   lsp-ui-doc-max-height 13

   lsp-ui-doc-frame-mode t       ;; I don't see any difference
   lsp-ui-doc-use-childframe t
   lsp-ui-doc-position 'at-point ;; 'at-point can't limit to buffer which is a good thing
   lsp-ui-doc-side 'right        ;; only with top|bottom
   lsp-ui-doc-alignment 'window  ;; only with top|bottom - use 'window to constrain to buffer
   lsp-ui-doc-border "green"     ;; requires restart
  ...
  (setq ;; 5. Sideline code actions
   ...
   ;; lsp-ui-sideline-actions-icon "/path/image.png"

   lsp-ui-sideline-ignore-duplicate t
   lsp-ui-sideline-show-symbol t
   lsp-ui-sideline-show-code-actions nil)

  (setq ;; 6. Sideline hover symbols
   lsp-ui-sideline-show-hover nil)

  (setq ;; 7. Modeline code actions
   lsp-modeline-code-actions-enable t)

  (setq ;; 8. Flycheck / flymake
   lsp-diagnostics-provider :auto)

  (setq ;; 9. Sideline diagnostics
   lsp-ui-sideline-show-diagnostics t)

  (setq ;; 10. Eldoc
   lsp-eldoc-enable-hover t)

  (setq ;; 11. Modeline diagnostics statistics
   lsp-modeline-diagnostics-enable t
   lsp-diagnostics-modeline-scope :workspace)

  (setq ;; 12. Signarure help
   ;; lsp-signature-auto-activate '(:on-trigger-char :on-server-request)
   )

  (setq ;; 13. Signature documentation
  ...
  (setq ;; 15. Completion item detail
   lsp-completion-show-detail t)

  (setq ;; 16. Completion item kind
...
(use-package! rustic
  ...
  (map! :map rustic-mode-map
        :localleader
        :desc "cargo add" :n "a" #'rustic-cargo-add
...
(use-package! sqlite
  :defer t
  :init
  (setq sql-sqlite-options '("-header" "-box")))
#+End_src
